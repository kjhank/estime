<?php get_header();
  ...
  <section class="page-section">
    ...
    <?php if (have_posts()) : ?>
      ...
      <?php
        if( have_rows('pricing-repeater') ):?>
        <section class="pricing">
          <h2>cennik</h2>
        <?php
          while ( have_rows('pricing-repeater') ) : the_row();
            echo '<h3>';
            the_sub_field('pricing-header');
            echo'</h3>';
            $table = get_sub_field( 'pricing-table' );
            if ( $table ) {
              echo '<table border="0" class="pricing-table">';
              if ( $table['header'] ) {
                echo '<thead>';
                echo '<tr>';
